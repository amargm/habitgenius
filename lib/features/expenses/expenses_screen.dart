...
class _ExpensesScreenState extends ConsumerState<ExpensesScreen>
              ...
              child: TabBarView(
                ...
                children: [
                  _TransactionsTab(
                    ...
                    accounts: accounts,
                    tier: tier,
                    onAdd: () => _onAddTransaction(context, tier,
  ...
  void _onAddTransaction(BuildContext context, UserTier tier,
      List<Transaction> txs, List<Account> accounts) {
    if (accounts.isEmpty) {
    ...
    if (txToday >= AppLimits.maxTransactionsPerDay(tier)) {
      UpgradePromptSheet.show(context,
          feature: 'More daily transactions');
      return;
    }
  ...
  void _onAddAccount(BuildContext context, UserTier tier,
...
class _TransactionsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final monthTotal =
        monthTxs.isEmpty ? 0.0 : monthTxs.map((t) => t.amount).reduce((a, b) => a + b);
  ...
  double _balance(Account a) {
    ...
    for (final tx in transactions) {
      if (tx.accountId == a.id) {
        bal += tx.type == TransactionType.income
            ? tx.amount
            : -tx.amount;
      }
...
class _TransactionSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_TransactionSheet> createState() =>
      _TransactionSheetState();
}

class _TransactionSheetState
    extends ConsumerState<_TransactionSheet> {
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      final sel = _type == t;
                      final c = t == TransactionType.income
                          ? AppColors.success
                          : AppColors.danger;
                      return Expanded(
